...
description: 최상위 1% 엘리트 성장주를 판별하는 Rule of 60과 바벨 포트폴리오 전략 실전 적용
---

# Rule of 60 🚀 엘리트 기업 및 바벨 포트폴리오 분류 (2Q26)

> **데이터 분석일(스크립트 실행 기준)**: 2026-07-11
> **데이터 기준(TTM)**: Trailing Twelve Months (최근 12개월 누적). 모든 수치는 데이터 분석일 시점에 각 기업이 시장에 발표한 가장 최신 4개 분기의 실적을 합산한 기준입니다.

![Rule of 60 Scatter Plot](/rule_of_60_chart.png)

본 리스트는 기존 룰 오브 40 달성 기업 중 **'Rule of 60 (총점 60 이상)'**이라는 더욱 엄격한 컷오프를 통과한 최상위 1% 엘리트 기업들만 선별한 결과입니다.
...
* **Cash Cows (현금 창출기):** 주로 **금융결제망(Financial Services), 부동산 리츠(Real Estate), 빅테크 플랫폼(Communication Services)** 위주로 포진되어 있습니다. 비자(V), 마스터카드(MA), 메타(META), 브로드컴(AVGO) 등은 이미 완벽한 해자(Moat)를 구축하여 성장률은 10~30%대로 안정화되었지만, 영업이익률이 무려 50~80%에 육박하며 어마어마한 잉여현금(FCF)을 자사주 매입과 배당으로 주주들에게 환원하고 있습니다.

### 🇰🇷 한국 시장 분석 (반도체 턴어라운드와 증권주의 약진)
* **Hyper-Growers:** 한국 증시의 가장 큰 특징인 **'메모리 반도체 사이클'**이 고스란히 반영되었습니다. 깊은 다운사이클에 따른 기저효과와 HBM 특수에 힘입어 SK하이닉스(198%), 삼성전자(69%) 등 반도체 투톱이 압도적인 성장을 기록하며 최상단을 차지했습니다. 또한 최근 코스피의 급격한 상승과 밸류업 기대감에 따른 거래대금 폭증에 힘입어 **증권사(미래에셋증권, NH투자증권, 키움증권 등)**들의 매출과 영업이익률이 뻥튀기(?)되며 대거 고성장 진영에 합류한 것이 특징입니다. 에이피알(APR), 삼양식품 같은 K-뷰티/K-푸드 수출 대장주들도 탄탄한 성장성을 증명했습니다.
