...
KCA는 전통적인 **DCA(Dollar Cost Averaging)**를 원화 기준으로 실행하기 위해 제가 이름 붙인 개인적 적립 규칙입니다. 시장과 환율을 예측하기보다 같은 원화 금액을 반복해서 투자하는 데 목적이 있습니다.
...
> [!NOTE] 원자료·산식·한계
> DCA의 기본 정의는 미국 SEC의 [Investor.gov 설명](https://www.investor.gov/introduction-investing/investing-basics/glossary/dollar-cost-averaging)을 따릅니다. KCA의 `N = M_KRW ÷ (P_USD × E)`는 매수 수량을 계산하는 산술식이며 기대수익 모형이 아닙니다. 환율 위험, 거래 비용, 세금, 가격 하락을 제거하지 않으며 별도의 학술 검증을 거친 전략도 아닙니다.
...
> 💡 **미리 설계된 무위(無爲)**
>
> 이 산식은 주가와 환율을 예측하지 않고 원화 기준 매수 금액을 일정하게 유지하기 위한 실행 규칙입니다. 주가와 환율이 항상 반대로 움직이는 것은 아니며 환차손을 제거하지도 않지만, 매수 때마다 환율을 판단해야 하는 부담을 줄이는 데 목적이 있습니다.
...
* **결론**:
  KCA 전략은 **자산 축적 초·중기에 주가와 환율을 매번 예측해야 하는 부담을 줄이기 위한 개인적 실행 규칙**입니다. 변동성과 손실을 제거하거나 장기 성과를 보장하지는 않지만, 저에게는 계획된 적립을 지속하는 데 도움이 되었습니다.
...
* 결과적으로 일부 투자자는 이 방식을 심리적으로 더 유지하기 쉽다고 느낄 수 있지만, 시장 스트레스가 사라지는 것은 아닙니다.

> ⏭️ **다음 단계로**: 원화 정액 매수는 수동으로도 가능하지만, 자주 실행할수록 계산과 주문이 번거로워집니다. 다음 페이지에서는 반복 작업을 줄이기 위해 제가 사용하는 **자동 투자 설정**과 점검 항목을 설명합니다.
