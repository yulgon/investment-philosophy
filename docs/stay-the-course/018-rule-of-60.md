...
![Rule of 60 멘탈 닻](/rule_of_60_anchor.png)
...
대신, 저는 자산의 대부분을 든든한 **'코어(Core) ETF'**로 방어하면서 초과 수익을 노리는 **'위성(Satellite) 개별 기업'**을 고를 때, 시장의 소음을 철저히 배제하고 오직 아래의 **3가지 객관적인 지표**만을 통해 기업을 검토합니다.

1. **SPMO (Invesco S&P 500 Momentum ETF) 리밸런싱 내역:** 실제 주가 상승 추세를 타는 주도주를 필터링하는 **'모멘텀(Momentum)'** 지표.
2. **SCHD (Schwab US Dividend Equity ETF) 리밸런싱 내역:** 재무 건전성과 배당을 통해 기업의 방어력을 필터링하는 **'퀄리티(Quality)'** 지표.
3. **Rule of 40 / 60 (룰 오브 40/60):** 비즈니스의 **'현재 실적(매출 성장률 + 영업이익률)'**이라는 순수한 팩트에 점수를 매겨 위성 기업을 최종 선택하는 핵심 룰.

내 자산의 중심(Core)을 모멘텀(SPMO)과 퀄리티(SCHD)로 균형 있게 굴리듯이, 개별 위성 기업(Satellite)을 고를 때도 이 3가지 지표를 통해 확고한 균형감을 유지하는 것입니다. 
...
제 부족한 경험 속에서, 폭락장 속에서도 흔들리는 멘탈의 중심을 단단하게 잡아준 무거운 **'바벨(Barbell)'**이 바로 이 공식이었습니다. 룰 오브 40은 아래와 같습니다.

![Rule of 60 Barbell](/rule_of_60_barbell.png)
